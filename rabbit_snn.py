...
from ml_genn.neurons import LeakyIntegrateFire, LeakyIntegrate, SpikeInput
from ml_genn.connectivity import Dense
from ml_genn.readouts import SumVar
...
NUM_EPOCHS = 50  # FIX: single source of truth for epoch count
...
def collect_training_data():
    ...
    while True:
        ...
        # FIX: check ret before accessing frame.shape
        if not ret:
        ...
        # FIX: only build spike array when we have motion data
        spike_array = np.zeros(NUM_PIXELS, dtype=np.float32)
        ...
        # FIX: draw bounding box BEFORE imshow so it actually appears
        if activePoints:
        ...
        # FIX: show live label counts so you know if your dataset is balanced
...
def build_network():
    ...
    with net:
        ...
        output = Population(LeakyIntegrate(tau_mem=20.0, readout=SumVar()), NUM_CLASSES)
...
def train(samples, labels):
    ...
    with compiled:
        compiled.train(
            {inp: samples},
            {output: labels},
            num_epochs=NUM_EPOCHS,  # FIX: use constant
            shuffle=True,
...
def inference():
    # FIX: guard against missing weights
    ...
    serialiser = Numpy(WEIGHTS_FILE)
    network.load((NUM_EPOCHS - 1,), serialiser)  # FIX: use constant
    ...
    with compiled:
        while True:
            ...
            # FIX: use same torch pipeline as collect so spikes match training data
            currentFrameTensor, changedPixels = torch_motion_frame(
                frame, currentFrameTensor, firstLoopPassed, threshold
            )
            ...
            # FIX: spike_data is bool, use it directly
...
if __name__ == "__main__":
    if "--collect" in sys.argv:
        ...
        # FIX: guard against missing training data
